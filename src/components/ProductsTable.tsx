...
import { deleteProduto } from '../Store/slices/salesSlice';
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden p-4 sm:p-6 mb-8">
        ...
        <div className="inline-block min-w-full align-middle">
          <table className="min-w-full rounded-lg overflow-hidden border border-gray-200">
            <thead className="bg-gray-100">
              <tr>
                <th className="px-3 sm:px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Produtos</th>
                <th className="px-3 sm:px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Total</th>
                <th className="px-3 sm:px-6 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Média mensal</th>
                <th className="px-3 sm:px-6 py-3 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">Ações</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {products.map((product: Produto) => (
                <tr key={product.id} className="hover:bg-gray-50">
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{product.produto}</td>
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-800">{getTotalSales(product)}</td>
                  <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-800">{getMonthlyAverageSales(product)}</td>
      ...
      {showAddForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow-xl w-full max-w-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg sm:text-xl font-semibold">{editingProduct ? 'Editar Produto' : 'Adicionar Novo Produto'}</h2>
              <button onClick={() => {
